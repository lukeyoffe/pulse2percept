"""`Dynaphos2022Model`, `Dynaphos2022Spatial`, `Dynaphos2022Temporal`
   [Dynaphos2022]_"""
import numpy as np
...
class Dynaphos2022Spatial(SpatialModel):
    """Spatial response model of [Dynaphos2022]_

    Implements the spatial response model described in [Dynaphos2022]_, which
    ...
    .. note::
        ...
        Use :py:class:`~pulse2percept.models.Dynaphos2022Model` if you want both
    ...
    def get_default_params(self):
        ...
        base_params = super(Dynaphos2022Spatial, self).get_default_params()
    ...
    def predict_percept(self, implant, t_percept=None):
        if not np.all([isinstance(e, DiskElectrode)
            ...
            raise TypeError("The Dynaphos2022 spatial model only supports "
                            "DiskElectrode arrays.")
        return super(Dynaphos2022Spatial, self).predict_percept(
            implant, t_percept=t_percept
        )


class Dynaphos2022Temporal(TemporalModel):
    """Temporal model of [Dynaphos2022]_

    Implements the temporal response model described in [Dynaphos2022]_, which
    ...
    def get_default_params(self):
        base_params = super(Dynaphos2022Temporal, self).get_default_params()
        params = {
...
class Dynaphos2022Model(Model):
    """[Dynaphos2022]_ Model

    Implements the model described in [Dynaphos2022]_, where percepts are
    circular and their brightness evolves over time.

    The model combines two parts:

    *  :py:class:`~pulse2percept.models.Dynaphos2022Spatial` is used to
    ...
    *  :py:class:`~pulse2percept.models.Dynaphos2022Temporal` is used to
    ...
    def __init__(self, **params):
        super(Dynaphos2022Model, self).__init__(spatial=Dynaphos2022Spatial(),
                                               temporal=Dynaphos2022Temporal(),
                                               **params)
